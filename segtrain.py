...
cfgs = {
    ...
    "gamma": 0.5,
    "datasize": 8,
    "runspath": "/media/ExtraDrive2/jianingq/train_runs",
...
class Train(BaseTrain):
    ...
    def init_model(self):
        ...
        self.criterion_seg = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(20.).to('cuda'))
    ...
    def loss(self, outputs, labels):
        loss = self.criterion_seg(outputs[:,:self.n_class,:,:], labels)
        return loss
    ...
    def train(self):
        ...
        for epoch in range(self.cfgs["epochs"]):
            ...
            if epoch % 10 == 0 and self.datasize > 8:
                self.val(epoch)
    ...
    def val(self, epoch):
        ...
        for iter, batch in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                loss = self.loss(outputs, labels, variance, mask)
                losses.append(loss.item())
...
#             if epoch % 100 == 0:
#                 hm = torch.sigmoid(outputs[0, -1, :, :]).data.cpu().numpy()
#                 self.val_writer.add_image('hm_%d' % iter, hm, epoch*num_batches+iter, dataformats="HW")
